...
*/
void BinaryReplacement::write_from_uChar(
  unsigned char uChar,
...
*/
void BinaryReplacement::write_file_count(
  int file_count,
  unsigned char &current_byte,
  int current_bit_count,
  FILE *compressed_fp
...
void BinaryReplacement::write_file_size(
  long int size,
  unsigned char &current_byte,
  int current_bit_count,
  FILE *compressed_fp
...
void BinaryReplacement::write_file_name(
  char *file_name,
  std::string *str_arr,
  unsigned char &current_byte,
  int &current_bit_count,
...
void BinaryReplacement::write_the_file_content(
  FILE *original_fp,
  long int size,
  std::string *str_arr,
  unsigned char &current_byte,
...
int BinaryReplacement::this_is_not_a_folder(char *path){
...
long int BinaryReplacement::size_of_the_file(char *path){
  long int size;
...
void BinaryReplacement::count_in_folder(
  std::string path,
  long int *number,
  long int &total_size,
  long int &total_bits
...
void BinaryReplacement::write_the_folder(
  std::string path,
  std::string *str_arr,
  unsigned char &current_byte,
...
void BinaryReplacement::translate(std::string binary){
  long int number[256];
  ...
  original_fp=fopen(binary.c_str(),"rb");
  fread(temp,1,1,original_fp);

  int size_limit = std::min((long int) binary.size(), (long int) 4);
  std::string chars = "";
